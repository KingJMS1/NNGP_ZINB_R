...
getAD_ho = function(neardist, neardistM, N, M, phi,deltasq){
  # coords: n by 2 array,
  # phi, v, alpha: covariance parameter set
  # hold: whether AD is for holded locations or not
  ...
  AD = matrix(NA, nrow=M+1, ncol=N)
  # for every row of matrix A and D
  for (i in 1:N){ # i starts from 2 because first row of A = 0
    # distance among the neighbors
    temp_neardistM = matrix(NA, nrow=M,ncol=M)
    u = rep(NA, M)
