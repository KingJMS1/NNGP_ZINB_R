...
ZINB_NNGP = function(X, x, y, coords, nis, 
                     Vs, Vt, Ds, Dt, M = 10,
                     nsim, burn, thin=1,save_ypred=FALSE){
  ...
  ####### prior mean for a, b, c, d  ######
  mu_a = mu_c = rep(0, N) # spatial
  mu_b = mu_d = rep(0, n_time_points) # temporal
  
  ####### kernel hyperparameters  ######
  a_phi=8; b_phi=1 # spatial kernel: phi~Gamma(shape=a,rate=b), mean=a/b
  a_sigmas=2; b_sigmas=0.25 # spatial kernel: sigmasq~IG(shape=a, scale=b), mean=b/(a-1)
  a_lt=1; b_lt=1 # temporal kernel: l~Gamma(a,b)
  a_sigmat=2; b_sigmat=0.04 # temporal kernel: sigmasq~IG(a, b)
  
  ####### random noise hyperparameters ######
  a_eps1s = 5; b_eps1s = 0.01  # sigmasq~IG(a,b)
  a_eps2s = 5; b_eps2s = 0.01
  a_eps1t = 5; b_eps1t = 0.01  # sigmasq~IG(a,b)
  a_eps2t = 5; b_eps2t = 0.01
  ...
  sd_sigma_T=0.1 #sigma1t~normal(sigma1t,sd_sigma_T)
  ...
  for (i in 1:nsim){
    ...
    Kt_bin = sigma1t^2*exp(-Dt/(l1t^2))
    Kt_nb = sigma2t^2*exp(-Dt/(l2t^2))
    Sigma0_bin.inv = as.matrix(Matrix::bdiag(FastGP::rcppeigen_invert_matrix(Ks_bin),
                                             FastGP::rcppeigen_invert_matrix(Kt_bin)))
    Sigma0_nb.inv = as.matrix(Matrix::bdiag(FastGP::rcppeigen_invert_matrix(Ks_nb),
    ...
    Ks_bin_inv=FastGP::rcppeigen_invert_matrix(Ks_bin)
    T0_s1 =  Matrix::bdiag(FastGP::rcppeigen_invert_matrix(Ks_bin),
                           1/(sigma_eps1s^2)*diag(n))
    T0_s2 =  Matrix::bdiag(FastGP::rcppeigen_invert_matrix(Ks_nb),
                           1/(sigma_eps2s^2)*diag(n))
    T0_t1 =  Matrix::bdiag(FastGP::rcppeigen_invert_matrix(Kt_bin),
                           1/(sigma_eps1t^2)*diag(n_time_points))
    T0_t2 =  Matrix::bdiag(FastGP::rcppeigen_invert_matrix(Kt_nb),
                           1/(sigma_eps2t^2)*diag(n_time_points))
    ...
    NN.matrix <- NNMatrix(coords = coords, n.neighbors = M,
                          n.omp.threads = 1, search.type = "cb") # replace the input NNmatrix
    AD <- getAD(neardist = NN.matrix$NN_dist,  neardistM = NN.matrix$NN_distM, 
                N = n, M = M, phi = phi_bin) 
    dim(AD)
    Dm <- AD[M + 1, ]
    ...
    DIA <-
      as.matrix(sparseMatrix(i = ind_x, j = (ind_y),
                   x = c(-na.omit(as.vector(AD[-(M + 1), ])), rep(1, n))) / sqrt(Dm))
    dim(DIA)
    rho_inv = t(DIA) %*% DIA
    ...
    # if(l2t_star < 4 & l2t_star > 0 & sigma2t_star > 0){
    ...
    if (i> burn & i%%thin==0) {
      j<-(i-burn)/thin
      Alpha[j,]<-alpha; Beta[j,]<-beta # fixed effects
      A[j,] = a; B[j,] = b; C[j,] = c; D[j,] = d # random effects
      Eps1s[j,]=eps1s; Eps2s[j,]=eps2s # random noise
      Eps1t[j,]=eps1t; Eps2t[j,]=eps2t # random noise
      L1t[j] = l1t; Sigma1t[j] = sigma1t # temporal hyperparameters
      L2t[j] = l2t; Sigma2t[j] = sigma2t # temporal hyperparameters
      Phi_bin[j] = phi_bin; Sigma1s[j] = sigma1s # spatial hyperparameters
      Phi_nb[j] = phi_nb; Sigma2s[j] = sigma2s # spatial hyperparameters
      Sigma_eps1s[j]=sigma_eps1s; Sigma_eps2s[j]=sigma_eps2s # random noise parameters
      Sigma_eps1t[j]=sigma_eps1t; Sigma_eps2t[j]=sigma_eps2t # random noise parameters
      R[j]<-r		
      R2[j]<-r2
      if (save_ypred==TRUE) {
        Y_pred[j,] = y_pred
  ...
  results = list(Alpha = Alpha, Beta = Beta, A = A, B=B, C=C,D=D,
